
<?php
//creamos la class
class videojoc {
// función para establecer conexion con la base de datos
  public function connectar_bd ($servername,$username,$password)
  {
    try {
      $conn = new PDO("mysql:host=$servername;dbname=videojocs", $username, $password);
      // set the PDO error mode to exception
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    } catch(PDOException $e) {
      echo "Connection failed2: " . $e->getMessage(); // imprime info de errores 
    }
    return $conn;
}
// función para insertar datos dentrode la base de datos
public function inserir ($servername,$username,$password,$nom, $data_llancament, $pegi, $id_desenvolupador, $id_plataforma)
{
    $conn = $this->connectar_bd($servername,$username,$password);
      try
      {
        $sql = "INSERT INTO videojoc (id, nom, data_llancament, pegi, id_desenvolupador, id_plataforma)
        VALUES (NULL, '$nom', '$data_llancament', '$pegi', '$id_desenvolupador', '$id_plataforma');";
        $conn->exec($sql); //ejecutamos la consulta sql en la abse de datos
        $last_id = $conn->lastInsertId();
} catch(PDOException $e) {
        echo $sql . "<br>" . $e->getMessage(); // imprime info de errores 
  }
  $conn = null; //cierra conexión con la base de datos
}

// función para consultar datos dentrode la base de datos

public function consultaTots ($servername, $username,$password)
{
    $conn = $this->connectar_bd($servername,$username,$password);

    try {
       $stmt = $conn->prepare("SELECT * FROM videojoc");
       $result = $stmt->execute();
       $conn=null;
       return($stmt); 
    }

    catch(PDOException $e) {
    echo "Error: " . $e->getMessage();
    }
}
// función para consultar datos por el nombre  dentro de la base de datos
public function consultanom ($servername, $username,$password,$nom)
{
    $conn = $this->connectar_bd($servername,$username,$password);

    try {
       $stmt = $conn->prepare("SELECT id, nom, data_llancament, pegi, id_desenvolupador, id_plataforma FROM videojoc where nom = '$nom'");
       $result = $stmt->execute();
       $conn=null;
       return($stmt); 
    }

    catch(PDOException $e) {
    echo "Error: " . $e->getMessage();
    }
} 
// función para consultar datos por la fecha  dentro de la base de datos
public function consultdata($servername, $username, $password, $data_llancament)
{
    $conn = $this->connectar_bd($servername, $username, $password);

    try {
        $stmt = $conn->prepare("SELECT id, nom, data_llancament, pegi, id_desenvolupador, id_plataforma FROM videojoc WHERE data_llancament = '$data_llancament'");
        $stmt->execute();
        // Obtener los datos como un array asociativo
        $arrayValues = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $conn = null;
        return $arrayValues;
    } catch (PDOException $e) {
        echo "Error: " . $e->getMessage();
        return false;
    }
}
// función para consultar datos por el desenvolupador  dentro de la base de datos
public function consultdesenvolupador($servername, $username, $password, $id_desenvolupador)
{
    $conn = $this->connectar_bd($servername, $username, $password);

    try {
        $stmt = $conn->prepare("SELECT id, nom, data_llancament, pegi, id_desenvolupador, id_plataforma FROM videojoc WHERE id_desenvolupador = '$id_desenvolupador'");
        $stmt->execute();
        $arrayValues = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $conn = null;

        return $arrayValues;
    } catch (PDOException $e) {
        echo "Error: " . $e->getMessage();
        return false;
    }
}


// función para modificar datos dentrode la base de datos
function modificar ($servername,$username,$password, $id,$nom, $data_llancament, $pegi, $id_desenvolupador, $id_plataforma)
{
  $conn = $this->connectar_bd($servername,$username,$password);
  try {
    $sql = "UPDATE videojoc SET nom='$nom', data_llancament='$data_llancament', pegi='$pegi', id_desenvolupador='$id_desenvolupador', id_plataforma='$id_plataforma' WHERE id='$id'";
  $stmt = $conn->prepare($sql);
  $conn->exec($sql);
} catch(PDOException $e) {
  echo $sql . "<br>" . $e->getMessage();
}

$conn = null;

}
// función para eliminar datos dentrode la base de datos
function eliminar ($servername,$username,$password, $id)
{
  $conn = $this->connectar_bd($servername,$username,$password);

try {
  $sql = "DELETE FROM videojoc WHERE id='$id'";
  $conn->exec($sql); //ejecutamos la consulta sql en la abse de datos
} catch(PDOException $e) {
  echo $sql . "<br>" . $e->getMessage();
}

$conn = null;
}


}